...
def get_ci_bins(source_trim, sample_rate):
    ...
    edges = numpy.logspace(
        math.log10(bass_cut_off_band), math.log10(window_size), num=11, dtype=int
    )
...
def calculate(source_file_name):
    sample_rate = 16000
    source_data = common.load_audio_file_as_numpy_array(source_file_name, sample_rate)
    window_size = 2048
    ...
    for offset in range(
        ...
        bins = get_ci_bins(source_data[offset : offset + window_size], sample_rate)
    ...
    for e in range(entry_count):
        ...
        entry["end_s"] = round((q + bin_20_width) * half_window_size / sample_rate)
    ...
    if table == []:
        ...
        result["ci_warning"] = (
            "Cacophony Index requires at least 20 seconds of audio, but only %d seconds of audio were provided."
            % p
        )
    return result
